...
export async function showAlerts() {
  ...
  tg.MainButton.hide();
  const { data: variants } = await supaCall(() =>
    getSupabase().from('variants').select('id, quantity, min_quantity, variant_name, products(name)').gt('min_quantity', 0)
  );
  const lowStock = variants?.filter(v => v.quantity <= v.min_quantity) || [];
  let html = `<div class="card"><h2>${t('alerts')}</h2>`;
  if (!lowStock.length) {
    html += `<div class="empty-state">🔔<br>${t('noData')}</div>`;
  } else {
    html += '<ul>';
    lowStock.forEach(v => {
      html += `<li><strong>${v.products.name}</strong> - ${v.variant_name || 'غير مسمى'} – ${v.quantity} (حد ${v.min_quantity})</li>`;
    });
    html += '</ul>';
  }
