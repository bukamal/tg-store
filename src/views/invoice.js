...
export function showInvoice(order, type='sale') {
  ...
  const html = `<div id="invoice-modal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:1000;display:flex;justify-content:center;align-items:center;">
    <div id="invoice-printable" style="background:#fff;padding:20px;border-radius:18px;max-width:400px;width:90%;color:#000;" dir="rtl">
      <h2>${title} #${id}</h2>
      <p>${t('date')}: ${new Date(created_at).toLocaleString()}</p>
      <p>${entityLabel}: ${entityName}</p>
      <table><tr><th>الصنف</th><th>كمية</th><th>سعر</th><th>إجمالي</th></tr>
        ${items.map(i=>`<tr><td>${sanitize(i.name)} ${sanitize(i.variantName||'')}</td><td>${i.quantity}</td><td>${formatCurrency(i.unit_price)}</td><td>${formatCurrency(i.quantity*i.unit_price)}</td></tr>`).join('')}
      </table>
      <p>الإجمالي: ${formatCurrency(total_amount)}</p><p>${t('discount')}: ${formatCurrency(discount||0)}</p>
      ${type==='sale'?`<p>${t('tax')}: ${formatCurrency(tax)}</p>`:''}
      <p><strong>المستحق: ${formatCurrency(net)}</strong></p>
      <p>${t('paid')}: ${formatCurrency(paid_amount)}</p>
      <p>${t('remaining')}: ${formatCurrency(remaining)}</p>
      <div style="text-align:center;margin-top:10px">
        <button class="btn btn-sm" id="print-invoice">${t('print')}</button>
        <button class="btn btn-sm" id="download-pdf-invoice">${t('downloadPDF')}</button>
        <button class="btn btn-sm btn-secondary" id="close-invoice">${t('close')}</button>
      </div>
    </div>
  </div>`;
  ...
  document.getElementById('print-invoice').addEventListener('click', ()=>{
    ...
    w.document.write(`<html dir="rtl"><head><title>${title} #${id}</title></head><body>${document.getElementById('invoice-printable').innerHTML}</body></html>`);
    w.print();
  });
