...
export async function showAnalytics() {
  ...
  document.getElementById('view').innerHTML = `<div class="card"><h2>📊 تحليلات</h2>
    <div class="stats-grid"><div class="stat-card"><strong>إجمالي المبيعات</strong><br/>${formatCurrency(totalSales)}</div><div class="stat-card"><strong>تكلفة المبيعات</strong><br/>${formatCurrency(totalCost)}</div><div class="stat-card"><strong>المصروفات</strong><br/>${formatCurrency(totalExp)}</div><div class="stat-card" style="color:${netProfit>=0?'green':'red'}"><strong>صافي الربح</strong><br/>${formatCurrency(netProfit)}</div></div>
    <canvas id="salesChart"></canvas><canvas id="topProductsChart"></canvas><button class="btn btn-outline" id="refresh-analytics">🔄 تحديث</button></div>`;
  new Chart(document.getElementById('salesChart'),{type:'line',data:{labels:days,datasets:[{label:'مبيعات (ل.س)',data:dayTotals,borderColor:'#007aff',backgroundColor:'rgba(0,122,255,0.1)',fill:true,tension:0.2}]},options:{plugins:{legend:{display:false}}}});
