...
export async function showSellForm() {
  ...
  document.getElementById('view').innerHTML = `<div class="card"><h2>${t('sell')}</h2><div style="display:flex;gap:8px"><select id="prod-select">${prodOptions}</select><input id="cart-qty" type="number" value="1" min="1" style="width:60px"/><button class="btn btn-sm" id="add-to-cart">أضف</button></div><table id="cart-table"><thead><tr><th>الصنف</th><th>سعر</th><th>كمية</th><th>إجمالي</th><th></th></tr></thead><tbody></tbody></table><p>${t('total')}: <span id="cart-total">0</span></p><div><label>${t('discount')}:</label><input id="discount" type="number" value="0" step="0.01"/></div><div><label>${t('tax')}:</label><input id="tax" type="number" value="0" step="0.01"/></div><div><label>${t('paid')}:</label><input id="paid-amount" type="number" value="0" step="0.01"/></div><p>${t('grandTotal')}: <strong id="grand-total">0</strong></p><div><label>${t('customer')}:</label><select id="customer-select">${custOptions}</select></div></div>`;
...
async function checkout() {
  ...
  try {
    ...
    const { data: order, error } = await supaCall(() => getSupabase().from('orders').insert({user_id:getCurrentUserId(),total_amount:total,discount,tax,paid_amount:paid,customer_id:custId}).select().single());
