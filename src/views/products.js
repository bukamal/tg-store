...
export async function showProducts() {
  window.currentRefreshFunction = showProducts; tg.MainButton.hide();
  const { data: products } = await supaCall(() => getSupabase().from('products').select('id, name, variants(id, variant_name, purchase_price, selling_price, quantity, min_quantity)').order('name'));
...
export async function showAddProductForm() {
...
function addVariantRow() {
  ...
  row.innerHTML = `<input placeholder="اسم المتغير" class="v-name"/><input type="number" placeholder="سعر الشراء" class="v-purchase-price"/><input type="number" placeholder="سعر البيع" class="v-selling-price"/><input type="number" placeholder="الكمية" class="v-qty"/><input type="number" placeholder="حد التنبيه" class="v-min" value="0"/><button class="btn btn-sm btn-danger remove-variant">X</button>`;
...
async function updateProduct(productId) {
  ...
  tg.MainButton.disable(); tg.MainButton.showProgress();
  try {
    await supaCall(() => getSupabase().from('variants').delete().eq('product_id', productId));
    const newVariants = variants.map(v => ({ ...v, product_id: productId, user_id: getCurrentUserId() }));
