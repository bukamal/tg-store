...
async function saveCustomer() {
  ...
  try {
    await supaCall(() => getSupabase().from('customers').insert({
      user_id: getCurrentUserId(), name, phone: document.getElementById('cust-phone').value, notes: document.getElementById('cust-notes').value
    }));
...
window.showEntityPayments = async (type, entityId) => {
  ...
  const { data: invoices } = await supaCall(() => getSupabase().from(table)
    .select('id, created_at, grand_total, paid_amount, total_amount')
    .eq(entityField, entityId)
    .neq('paid_amount', getSupabase().raw('COALESCE(grand_total, total_amount)'))
    .order('created_at'));
