...
export async function showAddPurchaseForm() {
  ...
  document.getElementById('view').innerHTML = `<div class="card"><h3>تسجيل شراء جديد</h3><div style="display:flex;gap:8px"><select id="purchase-variant">${prodOptions}</select><input id="purchase-qty" type="number" value="1" min="1" style="width:60px"/><input id="purchase-cost" type="number" step="0.01" placeholder="تكلفة الوحدة"/><button class="btn btn-sm" id="add-to-purchase-cart">أضف</button></div><table id="purchase-cart-table"><thead><tr><th>الصنف</th><th>تكلفة</th><th>كمية</th><th>إجمالي</th><th></th></tr></thead><tbody></tbody></table><p>الإجمالي: <span id="purchase-total">0</span></p><div><label>حسم مكتسب:</label><input id="purchase-discount" type="number" step="0.01" value="0"/></div><div><label>المدفوع:</label><input id="purchase-paid" type="number" step="0.01" value="0"/></div><div><label>${t('supplier')}:</label><select id="purchase-supplier">${supplierOpts}</select></div><textarea id="purchase-note" placeholder="ملاحظات"></textarea></div>`;
...
async function completePurchase() {
  ...
  try {
    ...
    const { data: purchase, error } = await supaCall(() => getSupabase().from('purchases').insert({ user_id: getCurrentUserId(), supplier_id: supplierId, total_cost: totalCost, discount, paid_amount: paidAmount, note }).select().single());
