...
export async function showCashRegister() {
  ...
  tg.MainButton.hide();
  const { data: transactions } = await supaCall(() =>
    getSupabase().from('cash_register').select('*').order('created_at', { ascending: false })
  );
...
async function saveCashTransaction(type) {
  ...
  try {
    await supaCall(() =>
      getSupabase().from('cash_register').insert({ user_id: getCurrentUserId(), type, amount, reference_type: 'manual', note })
    );
...
async function exportCashCSV() {
  ...
  exportToCSV(
    'الصندوق.csv',
    ['النوع', 'المبلغ', 'ملاحظات', 'التاريخ'],
    data.map(t => [t.type === 'deposit' ? 'إيداع' : 'سحب', t.amount, t.note || '', new Date(t.created_at).toLocaleString()])
  );
}
