...
export async function showExpenses() {
  ...
  const workerTotals = {};
  workers?.forEach(e => {
    const w = e.worker_name || 'بدون';
    workerTotals[w] = (workerTotals[w] || 0) + parseFloat(e.amount);
  });
...
window.deleteExpense = async id => {
  if (confirm(t('confirmDelete'))) {
    await supaCall(() => getSupabase().from('expenses').delete().eq('id', id));
...
async function saveExpense() {
  ...
  try {
    const { data: record, error } = await supaCall(() =>
      getSupabase()
        .from('expenses')
        .insert({ user_id: getCurrentUserId(), amount, category, worker_name: workerName, description, expense_date: date })
        .select('id')
        .single()
    );
