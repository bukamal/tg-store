...
(async () => {
  try {
    ...
    if (!viewEl) {
      alert('عنصر view غير موجود');
      return;
    }
    viewEl.innerHTML = '<div class="empty-state"><div class="emoji">⚡</div>جاري التحميل...</div>';

    // التحقق من initData بدقة
    const initData = tg.initData || '';
    if (!initData || initData.length < 10) {
      viewEl.innerHTML = '<div class="empty-state">❌ بيانات تيليجرام غير صالحة</div>';
      return;
    }
    ...
    document.getElementById('view').innerHTML =
      `<div class="card" style="color:red;">⚠️ خطأ غير متوقع: ${globalError.message}</div>`;
  }
})();
